...
(function(window, undefined) {
	Array.prototype.filter = function(fun) {
		...
		for(var res = [], thisArg = arguments.length >= 2 ? arguments[1] : void 0, i = 0; len > i; i++)
			if(i in t) {
				var val = t[i];
	...
	var rootJe,
		...
		core_rnotwhite = /\S/, //匹配任何非空白字符
		core_rspace = /\s+/, //匹配任何空白字符，包括空格、制表符、换页符，一次或者多次
		rtrim = /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, //
	...
	Je.extend({
		...
		each: function(obj, callback) {
			var name, i = 0,
				length = obj.length,
				isObj = length === undefined || Je.isFunction(obj);
			if(isObj) {
				for(name in obj) {
		...
		readData: function(elem, type, name, value, overwrite) {
			var cache = Je.cache,
				...
				index = Je.getCacheIndex(elem, !isRead);
			if(isRead) {
		...
		removeData: function(elem, type, name) {
			var data, cache = Je.cache,
				index = Je.getCacheIndex(elem);
		...
		map: function(elems, callback, arg) {
			var value, key, ret = [],
				i = 0,
				length = elems.length,
				isArray = elems instanceof Je || length !== undefined && typeof length === "number" && (length > 0 && elems[0] && elems[length - 1] || length === 0 || Je.isArray(elems));
			if(isArray) {
				for(; i < length; i++) {
	...
	function getWidthOrHeight(elem, name) {
		...
		if(Je.isWindow(elem[0])) {
			...
			var padding = name === "width" ? ["left", "right"] : ["top", "bottom"],
				ret = elem[0][camelCase("offset-" + name)];
			if(ret <= 0 || ret == null) {
...
(function(Je) {
	...
	Je.extend({
		...
		ajax: function(options) {
			...
			for(i in Je.ajaxConfig)
				if(typeof opts[i] === "undefined") opts[i] = Je.ajaxConfig[i];
